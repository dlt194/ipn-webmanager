...
export function LicensesOverviewClient(props: { systemId: string }) {
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `/api/server-configs/${encodeURIComponent(props.systemId)}/licenses`,
          { cache: "no-store" },
        );
        ...
        if (active) setRows(json?.licenses ?? []);
      } catch (e) {
        if (active)
    ...
    <Card className="p-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Breakdown by license key using quantity totals.
        </p>
      </div>
