...
import { TYPE_INFO_LABELS } from "./systems/[systemId]/extensions/extensions-types";
...
export function OverviewExtensionsClient(props: { systemIds: string[] }) {
        ...
        const results = await Promise.all(
          props.systemIds.map(async (id) => {
            const res = await fetch(
              `/api/server-configs/${encodeURIComponent(id)}/extensions`,
              { cache: "no-store" },
            );
    ...
    <Card className="p-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Combined breakdown by extension type.
        </p>
      </div>
